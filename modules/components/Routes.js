...
var Routes = React.createClass({
  ...
  propTypes: {
    ...
    persistData: React.PropTypes.bool.isRequired
  },
...
/**
 * Runs all willTransition* hooks, computes and sets the new state for routes,
 * and returns a promise that resolves after all didTransitionFrom and
 * getAsyncProps hooks finish and all components are re-rendered.
 */
...
/**
 * Runs the getAsyncProps hook of all handlers in parallel and returns
 * a promise that resolves after all props are set. getAsyncProps may
 * return a hash or a promise for a hash, the values of which may be
 * immediate or promises.
 */
function runGetAsyncPropsHooks(matches, query) {
  var promises = [];

  matches.forEach(function (match) {
    var handler = match.route.props.handler;

    if (!handler.getAsyncProps)
      return;

    var props = handler.getAsyncProps(match.params, query);

    if (props != null) {
      promise.push(
        typeof props.then === 'function'
          ? props.then(resolveProperties)
